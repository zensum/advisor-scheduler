(function (_, Kotlin) {
  ...
  Slot$Factory.prototype.get_61zpoe$ = function (time) {
    ...
    firstOrNull_6jwkkr$break: do {
      ...
      while (tmp$.hasNext()) {
        var element = tmp$.next();
        if (Kotlin.equals(time, element.time) || this.moment_0(time).add(1, 'ms').isBetween(this.moment_0(element.time, 'YYYY-MM-DD hh:mm'), this.moment_0(element.time, 'YYYY-MM-DD hh:mm').add(element.durationInMinutes, 'minutes'))) {
